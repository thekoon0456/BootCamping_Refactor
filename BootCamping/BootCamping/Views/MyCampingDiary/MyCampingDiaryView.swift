...
struct MyCampingDiaryView: View {
    ...
    var body: some View {
                    ...
                    LazyVStack {
                        ForEach(diaryStore.myDiaryUserInfoDiaryList, id: \.self) { userInfoDiary in
                            DiaryCellView(item: userInfoDiary,
                                          isShowingAcceptedToast: $isShowingAcceptedToast,
                                          isShowingBlockedToast: $isShowingBlockedToast)
                                .task {
                                    guard let index = diaryStore
                                        .myDiaryUserInfoDiaryList
                                        .firstIndex(where: { $0.diary.id == userInfoDiary.diary.id})
                                    else { return }
        ...
        .alert("노트 만들기에 실패했습니다.. 다시 시도해 주세요.",
               isPresented: $diaryStore.isError) {
